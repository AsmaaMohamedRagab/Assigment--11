...
function getWelcome() {
    ...
    for (var i = 0; i < allUsers.length; i++) {
        if (mail.value == allUsers[i].mail && password.value == allUsers[i].password) {
            ...
            document.querySelector(".homePage").classList.replace("d-none", "d-block")
            document.querySelector(".homePage h1").innerHTML = `Welcome ${allUsers[i].name}`
            document.querySelector(".notexist").classList.replace("d-block", "d-none")
        } else {
...
logoutbtn.addEventListener("click", function () {
    ...
    document.querySelector(".homePage").classList.replace("d-block", "d-none")
    document.querySelector(".notexist").classList.replace("d-block", "d-none")
})
